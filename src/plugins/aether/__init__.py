...
def _help_text() -> str:
    return (
        ...
        "/aether 刷新       刷新当前布局\n"
        "/aether 事件 <n>   处理未知事件\n"
        "/aether 密码 <pwd> Cookie 失效时继续登录（建议私聊）\n"
        "/aether 停止       在安全节点暂停"
    )
...
async def handle_aether(matcher: Matcher,bot: Bot,event: Event,args: Message = CommandArg(),) -> None:
    ...
    if action in {"快速", "fast"}:
        ...
        if len(parts) >= 2:
            await _start_preset(matcher,bot,event," ".join(parts[1:]),fast_mode=True,)
            return
    ...
    if action in {"开始", "start"}:
        ...
        if len(parts) >= 2:
            ...
            await _start_preset(matcher,bot,event," ".join(choice_parts),fast_mode=fast_mode,)
            return
        matcher.state["aether_waiting_preset"] = True
